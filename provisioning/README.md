...
This will generate
- a [pluging jar](target/provisioning-agent-1.11.16.jar) which maybe dropped into an Ontop server (into the lib folder)
...
```console
docker build -t tractusx/provisioning-agent:1.11.16 -f src/main/docker/Dockerfile .
```
...
docker run -p 8080:8080 \
  ...
  tractusx/provisioning-agent:1.11.16
````
