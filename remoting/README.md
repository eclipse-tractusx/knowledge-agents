...
This will generate
- a [standalone jar](target/remoting-agent-1.11.16.jar) containing all necessary rdf4j components to build your own repository server.
- a [pluging jar](target/original-remoting-agent-1.11.16.jar) which maybe dropped into an rdf4j server for remoting support.

### Run Locally

The standalone jar](target/remoting-agent-1.11.16.jar) contains an example application that runs a sample repository against a sample source

```console
java -jar target/remoting-agent-1.11.16.jar -Dorg.slf4j.simpleLogger.defaultLogLevel=DEBUG
```
...
```console
docker build -t tractusx/remoting-agent:1.11.16 -f src/main/docker/Dockerfile .
```
...
docker run -p 8081:8081 \
  ...
  tractusx/remoting-agent:1.11.16
````
