...
public class QueryExecutorBuilder extends ExecHTTPBuilder<QueryExecutor, QueryExecutorBuilder> implements QueryExecMod, QueryExecBuilder {
    ...
    public static QueryExecutorBuilder service(String serviceUrl) {
        return create().endpoint(serviceUrl);
    }
    ...
    @Override
    protected QueryExecutor buildX(HttpClient httpClient, Query queryActual, String queryStringActual, Context cxt) {
        return new QueryExecutor(serviceURL, queryActual, queryStringActual, urlLimit,
                httpClient, new HashMap<>(httpHeaders), Params.create(params), cxt,
